defmodule Bitcoin.Node.Storage.Engine.Postgres do
  ...
  def store_block(%Messages.Block{} = block_msg, %{height: height} = opts) do
    ...
    Repo.transaction(fn ->
      tx_with_hashes =
        block_msg.transactions
        |> Bitcoin.Util.pmap(fn tx ->
      ...
      txs =
        ...
        |> Enum.map(fn {tx_hash, tx} ->
          %{hash: tx_hash, version: tx.version, lock_time: tx.lock_time}
        end)
      ...
      outputs =
        ...
        |> Enum.map(fn {tx_hash, tx} ->
          ...
          |> Enum.map(fn {output, idx} ->
            %{tx_hash: tx_hash, index: idx, value: output.value, pk_script: output.pk_script}
          end)
        end)
        |> List.flatten()
