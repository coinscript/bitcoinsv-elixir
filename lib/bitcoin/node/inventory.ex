defmodule Bitcoin.Node.Inventory do
  ...
  def seen(%Types.InventoryVector{reference_type: :msg_block, hash: hash}),
    do: seen({:block, hash})
  ...
  def handle_info(:cleanup, state) do
    ...
    |> Map.put(
      :items,
      state.items
      |> Enum.sort_by(fn {_k, i} -> i[:time] end)
      |> Enum.reverse()
  ...
  def handle_cast({:add, {:block, block}}, state) do
    ...
    state =
      case Storage.store_block(block) do
        :ok ->
        ...
        {:error, error} ->
          Logger.error(
            "Failed to store block #{hash |> Bitcoin.Util.hash_to_hex()} #{error |> inspect}"
          )
  ...
  def check_sync(state) do
    waiting_count =
      state.items
      |> Enum.filter(fn {{item_type, _hash}, item} ->
        item_type == :block && item[:status] == :waiting
      end)
      |> Enum.count()
  ...
  def check_orphans(state) do
    ...
    |> Map.put(
      :items,
      state.items
      |> Enum.map(fn {{item_type, hash} = key, item} ->
        item =
          cond do
            item_type == :block && item.status == :orphan ->
              case Storage.store_block(item[:item]) do
                ...
                {:error, error} ->
                  Logger.error(
                    "Failed to store block #{hash |> Bitcoin.Util.hash_to_hex()} #{
                      error |> inspect
                    }"
                  )
  ...
  def block_locator_hashes(hashes, height, _multiplier, _count) when height < 1,
    do: [@genesis_hash | hashes] |> Enum.reverse()

  # Fierst add 10 hashes of the latest blocks
  def block_locator_hashes(hashes, height, multiplier, count) when count <= 10,
    do:
  ...
  def block_locator_hashes(hashes, height, multiplier, count),
    do:
      block_locator_hashes(
        [hash_for_height(height) | hashes],
        height - multiplier,
        multiplier * 2,
        count + 1
      )
  ...
  defp hash_for_height(height),
    do: Storage.get_blocks_with_height(height) |> List.first() |> Bitcoin.Block.hash()
  ...
  defp item_to_inv_vector({:tx, hash}),
    do: %Types.InventoryVector{reference_type: :msg_tx, hash: hash}

  defp item_to_inv_vector({:block, hash}),
    do: %Types.InventoryVector{reference_type: :msg_block, hash: hash}
end
