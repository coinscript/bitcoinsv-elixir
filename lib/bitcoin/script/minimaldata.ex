defmodule Bitcoin.Script.Minimaldata do
  ...
  defmacro __using__(_opts) do
    quote do
      ...
      def run(stack, [:OP_PUSHDATA1, <<byte>> | _script], %{flags: %{minimaldata: true}} = _opts)
          when byte in 0..10,
          # TODO script test cases don't cover
          do: {:error, :minimaldata}
      ...
      def run(stack, [:OP_PUSHDATA1, data | _script], %{flags: %{minimaldata: true}} = _opts)
          when byte_size(data) < 75,
          do: {:error, :minimaldata}

      # Pushing 76 to 255 bytes must use OP_PUSHDATA1
      # Pushing 256 to 520 bytes must use OP_PUSHDATA2.
      def run(stack, [:OP_PUSHDATA2, data | _script], %{flags: %{minimaldata: true}} = _opts)
          when byte_size(data) < 256,
          do: {:error, :minimaldata}
