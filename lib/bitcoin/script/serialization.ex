defmodule Bitcoin.Script.Serialization do
  ...
  def parse(_script, <<@op_pushdata2, size::unsigned-little-integer-size(16), _bin::binary>>)
      when size > @max_element_size,
      do: @invalid
  ...
  def parse(_script, <<@op_pushdata4, size::unsigned-little-integer-size(32), _bin::binary>>)
      when size > @max_element_size,
      do: @invalid
  ...
  def parse_string_word("OP_" <> _ = op_name),
    do: op_name |> String.to_atom() |> parse_string_op_validate
  ...
  def parse_string2_word("'" <> bin),
    do: bin |> :binary.part(0, byte_size(bin) - 1) |> to_binary_word
