defmodule Bitcoin.Tx do
  ...
  def total_output_value(%Messages.Tx{} = tx, _opts \\ %{}),
    do: tx.outputs |> Enum.reduce(0, fn out, acc -> acc + out.value end)
  ...
  # TODO tests
  def fee(%Messages.Tx{} = tx, opts \\ %{}),
    do: total_input_value(tx, opts) - total_output_value(tx, opts)
  ...
  * `:previous_outputs` - already fetched previous outputs in the form of a map
  ...
  def validate(%Messages.Tx{} = tx, %{} = opts \\ %{}) do
    Bitcoin.Util.run_validations([
      ...
      fn ->
        tx.inputs
        |> Enum.with_index()
        |> Enum.reduce(:ok, fn {input, input_number}, result ->
          case result do
            :ok ->
  ...
  defp validate_input(tx, input, input_number, opts) do
    case find_previous_output(input, opts) do
      ...
      prev_out ->
        case Bitcoin.Script.verify_sig_pk(
               input.signature_script,
               prev_out.pk_script,
               opts |> Map.merge(%{tx: tx, input_number: input_number})
             ) do
          true ->
            :ok

          false ->
            # only saves if enabled in config
            save_as_test_case(tx, opts)
  ...
  defp find_previous_output(input, opts) do
    ...
    case Bitcoin.Node.Storage.get_txout(hash, index) do
      ...
      nil ->
        case opts[:block] &&
               Enum.find(opts[:block].transactions, fn tx -> Bitcoin.Tx.hash(tx) == hash end) do
  ...
  defp save_as_test_case(tx, opts) do
    if (Application.get_env(:bitcoin, :util) || [])[:save_as_test_case] do
      Logger.warn(
        "TX validation failed for #{tx |> hash |> Bitcoin.Util.hash_to_hex()}, saving context to test/data/auto"
      )
      ...
      json =
        [
          ...
          |> Enum.map(fn i ->
            [
              i.previous_output.hash |> Bitcoin.Util.hash_to_hex(),
              i.previous_output.index,
              find_previous_output(i, opts).pk_script |> Binary.to_hex()
            ]
          end),
      ...
      "test/data/auto/#{@network}/tx_#{tx |> hash |> Bitcoin.Util.hash_to_hex()}.json"
      |> File.write(json)
    else
      :ok
    end
