defmodule Bitcoin.Protocol.Types.InventoryVector do
  @inventory_vector_reference_types %{
    # 0 - Any data of with this number may be ignored
    0 => :error,
    # 1 - Hash is related to a transaction
    1 => :msg_tx,
    # 2 - Hash is related to a data block
    2 => :msg_block,
    # 3 - Hash of a block header; identical to MSG_BLOCK. When used in a getdata message, this indicates the reply should be a merkleblock message rather than a block message; this only works if a bloom filter has been set.
  ...
  def parse_stream(
        <<type_id::unsigned-little-integer-size(32), hash::bytes-size(32),
          remaining_stream::binary>>
      ) do
    {%__MODULE__{
       reference_type: type_id |> get_type_name,
       hash: hash
