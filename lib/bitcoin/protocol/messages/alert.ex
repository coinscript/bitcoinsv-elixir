defmodule Bitcoin.Protocol.Messages.Alert do
  ...
  @type t :: %Bitcoin.Protocol.Messages.Alert{
          ...
          # 64-bit unsigned Integer, native
          relay_until: non_neg_integer,
          # 64-bit unsigned Integer, native
  ...
  def parse(data) do
    ...
    <<alert_payload::bytes-size(alert_payload_bytes), alert_signature_payload::binary>> =
      alert_data
    ...
    {set_cancel, set_cancel_payload} =
      Bitcoin.Protocol.Types.IntegerArray.parse_stream(remaining_payload)
    ...
    {status_bar, status_bar_payload} =
      Bitcoin.Protocol.Types.VarString.parse_stream(comment_payload)

    {reserved, _} = Bitcoin.Protocol.Types.VarString.parse_stream(status_bar_payload)

    {alert_signature_bytes, alert_signature_payload} =
      Bitcoin.Protocol.Types.VarInteger.parse_stream(alert_signature_payload)
