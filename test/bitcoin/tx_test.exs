defmodule Bitcoin.TxTest do
  ...
  test "bitcoin core test cases" do
    ...
    vaild =
      File.read!("test/data/tx_valid.json") |> Poison.decode!() |> Enum.map(fn x -> {true, x} end)

    invalid =
      File.read!("test/data/tx_invalid.json")
      |> Poison.decode!()
    ...
    txs =
      (vaild ++ invalid)
      # remove comments
      |> Enum.filter(fn {_result, list} -> length(list) > 1 end)
    ...
    rets =
      ...
      |> Enum.map(fn {result, [prevouts, tx_serialized, flags]} ->
        ...
        tx_result =
          Bitcoin.Tx.validate(tx, %{
            previous_outputs: prepare_prevouts(prevouts, parser),
            flags: flags
          }) == :ok
  ...
  File.read!("test/data/tx_mainnet.json")
  ...
  with {:ok, list} <- File.ls("test/data/auto/#{@network}") do
    list
  ...
  def prepare_prevouts(prevouts, prevout_parser) do
    prevouts
    |> Enum.map(fn [prevout_hash, prevout_index, prevout_pk_script] ->
