defmodule Bitcoin.Block.ValidationTest do
  ...
  test "hash_below_target" do
    ...
    assert @block1 |> Map.put(:nonce, 1) |> Bitcoin.Block.Validation.hash_below_target() ==
  ...
  test "coinbase value" do
    ...
    assert block |> Bitcoin.Block.Validation.coinbase_value(%{height: 210_000}) ==
             {:error, :reward_too_high}

    assert block |> Bitcoin.Block.Validation.coinbase_value(%{height: 250_000}) ==
             {:error, :reward_too_high}
  end
  ...
  test "merkle root" do
    ...
    assert @block1
           |> Map.put(:merkle_root, @block1.merkle_root |> Binary.reverse())
           |> Bitcoin.Block.Validation.merkle_root() == {:error, :merkle_root_invalid}
  end
end
